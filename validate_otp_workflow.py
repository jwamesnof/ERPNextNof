...
def step0_list_sales_orders() -> List[Dict[Any, Any]]:
    ...
    for idx, order in enumerate(orders, 1):
        ...
        print(
            f"  {idx:2d}. {order['name']:20s} | {customer:30s} | Delivery: {delivery} | ${total:,.2f} ({status})"
        )
...
def select_sales_order(orders: List[Dict[Any, Any]]) -> str:
    ...
    while True:
        ...
        try:
            ...
            print(
                f"❌ Invalid input. Enter a number (1-{len(orders)}) or order name (e.g., SAL-ORD-2026-00001)"
            )
...
def step1_fetch_sales_order_details(so_id: str) -> Dict[Any, Any]:
    ...
    print(
        f"   Grand Total: ${so_details['grand_total']:.2f}"
        if so_details.get("grand_total")
        else "   Grand Total: N/A"
    )
