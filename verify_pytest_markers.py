...
def main():
    ...
    for test_file in test_dir.rglob("test_*.py"):
        ...
        if category in results:
            results[category].append(
                {"file": str(rel_path), "has_marker": has_marker, "marker": marker_info}
            )
    ...
    # Summary
    total_with_markers = sum(1 for files in results.values() for f in files if f["has_marker"])
